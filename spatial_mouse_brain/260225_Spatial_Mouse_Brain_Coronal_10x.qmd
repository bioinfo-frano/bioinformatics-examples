...
<https://www.youtube.com/watch?v=L_7VdCeJ4Z8&list=PLOLdjuxsfI4MXCyKZDY0jKU-K6Uo4o1_d&index=2.>

**Data source: 10XGenomics, Mouse brain section (coronal)** <https://www.10xgenomics.com/datasets/mouse-brain-section-coronal-1-standard-1-0-0>
...
# Mitochondrial Gene Percentage incorporated into brain@meta.data
...
brain[["percent.rb"]] <- PercentageFeatureSet(brain, pattern = "^Rp[sl]")  # Elevated ribosomal RNA can indicate residual cytoplasmic RNA or stress.
...
# Identify spots with high ribosomal gene expression
WhichCells(brain, expression = percent.rb > 19) # only one spot "CTCTGCAGGCATTCTT-1"

# Summary statistics for key QC metrics
...
summary(brain$percent.rb) # Confim > 20

# The spot with percent.rb > 20 does not show low nCount_Spatial or high percent.mt.
# Therefore, this elevated ribosomal signal may not indicate a technical artifact or stress.
# Filtering may be skipped in this case.

# Violin plot for spatial counts +  Ribosomal Protein Gene Percentage (percent.rb)
VlnPlot(brain, features = c("nCount_Spatial", "nFeature_Spatial", "percent.mt", "percent.rb"), pt.size = 0.1)   + NoLegend()

# Spatial visualization of percent.rb across the brain section
p1 <- SpatialFeaturePlot(brain, features = 
                     "percent.rb", alpha = c(0.1, 2)) + theme(legend.position = "right")

# Spatial visualization of nCount_Spatial across the brain section
p2 <- SpatialFeaturePlot(brain, features = 
                     "nCount_Spatial") + theme(legend.position = "right")

# Spatial visualization of nFeature_Spatial across the brain section
p3 <- SpatialFeaturePlot(brain, features = 
                     "nFeature_Spatial") + theme(legend.position = "right")

# Spatial visualization of nFeature_Spatial across the brain section
p4 <- SpatialFeaturePlot(brain, features = 
                     "percent.mt") + theme(legend.position = "right")
...
range(brain@meta.data$percent.rb) # 3.518981 23.129093
# normally, the brain has a high % of ribosome
...
# Save
# ggsave("~/path/to/your/data/Spatial_transcript_Visium_mouse_brain.png", final_layout, width = 16, height = 8, dpi = 600)
...
# Show only the first 6 genes (rows) and first 6 spots (columns)
print(as.matrix(head(brain@assays$SCT@data[, 1:6])), digits = 3)
...
# Save DEG results
# write.csv(all_markers, "~/path/to/your/data/Spatial_Data_MouseBrainCoronal/Spatial_mouse_brain_DEG_all_clusters.csv")
...
# Save
# ggsave("~/path/to/your/data/Spatial_highest_DEG_Visium_mouse_brain.png", cluster3_cluster1_highestDEG, width = 16, height = 8, dpi = 600)
...
# ggsave(
#   filename = "~/path/to/your/data/Spatial_brain_mouse_go_enrichment_highres.png",
#   plot = go_plot,
...
# Save 'H&E (p9)'
# ggsave("~/path/to/your/data/Spatial_Visium_mouse_brain_H&E.png", p9, width = 16, height = 8, dpi = 600)

# Save 'UMAP_regions (p7)'
# ggsave("~/path/to/your/data/Spatial_Visium_mouse_brain_UMAP_regions.png", p7, width = 16, height = 8, dpi = 600)

# Save 'patch1'
# ggsave("~/path/to/your/data/Spatial_Visium_mouse_brain_regions_patch.png", patch1, width = 16, height = 8, dpi = 600)

# Save 'Brain_regions (p8)'
# ggsave("~/path/to/your/data/Spatial_Visium_mouse_brain_regions.png", p8, width = 16, height = 8, dpi = 600)

# Save 'Brain_regions (p8)'
# ggsave("~path/to/your/data/Spatial_Visium_mouse_brain_regionsNoLegends.png", p8, width = 16, height = 8, dpi = 600)
...
# Save 'ft_region'
# Convert flextable to a grob
# ft_grob2 <- flextable::gen_grob(ft_region)
# ggsave("~path/to/your/data/ft_region_brainregions_descriptive_table.png", ft_grob2, width = 10, height = 8, dpi = 600)

# view(brain@meta.data)

# Render flextable for regions
flextable::htmltools_value(ft_region)
...
# Save 'ft_markers'
# Convert flextable to a grob
# ft_grob2 <- flextable::gen_grob(ft_markers)
# ggsave("~path/to/your/data/ft_markers_brainregions_Top5_predictive_genes_table.png", ft_grob2, width = 10, height = 8, dpi = 600)

# Render flextable for markers
flextable::htmltools_value(ft_markers)

# view(brain@meta.data)

# Save Seurat object 'with markers'brain'
# saveRDS(brain, file = "~/path/to/your/data/Spatial_transcript_Visium_mouse_brain_regions.rds")
...
# writeH5AD(
#   sce, 
#   file = "~path/to/your/data/Spatial_Data_MouseBrainCoronal/brain_metadata.h5ad",
...
# 
# # Save to H5AD
# writeH5AD(sce, file = "~/path/to/your/data/brain_metadata_corrected.h5ad", verbose = TRUE)
# 
...
# Save to H5AD
# writeH5AD(spe, file = "~/path/to/your/data/brain_metadata_corrected3_withCoordinatesRegions.h5ad", verbose = TRUE)
...
# library(zellkonverter)
# spe_check <- readH5AD("~/path/to/your/data/brain_metadata_corrected3_withCoordinatesRegions.h5ad")
